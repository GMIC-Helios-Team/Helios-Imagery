...
import Image from 'next/image';  // Import Image component from Next.js or your preferred library
...
const CarouselComponent = () => {
  ...
  const [flippedCards, setFlippedCards] = useState(Array(people.length).fill(false));

  // State to track the toggle (on or off)
  ...
  const handleToggleChange = () => {
    ...
    // Flip all cards when toggle is switched on
    setFlippedCards(Array(people.length).fill(!toggleOn));
  };
  ...
  return (
    <div className="carousel-container">
       {/* Toggle switch to flip all cards */}
       <div className="toggle-container">
        <label className="toggle-switch">
          <input type="checkbox" checked={toggleOn} onChange={handleToggleChange} />
          <span className="slider">
            <span className="slider-text">{toggleOn ? "DEP" : "Helios"}</span>
          </span>
        </label>
      </div>
      <Carousel responsive={responsive} swipeable infinite	arrows={false}  infinite={true} autoPlay={true} autoPlaySpeed={90000000}>
          ...
          return (
            ...
            >
              <div className="flip-card-inner">
                ...
                <Image src={profile.image} alt={profile.name} width={180} height={180} className="bio-image"/>                  
                  <h5>{profile.name}</h5>
